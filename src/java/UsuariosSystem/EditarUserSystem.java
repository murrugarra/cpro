 ...
 * @author user 1
 */
@WebServlet(name = "EditarUserSystem", urlPatterns = {"/EditarUserSystem"})
public class EditarUserSystem extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditarUserSystem</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EditarUserSystem at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sesion.getAttribute("usuario") == null){
          response.sendRedirect("CerrarSesion");
            ...
            Integer DniEdit = Integer.parseInt(request.getParameter("Dni"));  
            Integer DniActual = Integer.parseInt(request.getParameter("CodUser"));   
            ...
            try (Connection conexion = newCon.getConnection()) {
                ...
                cs = conexion.prepareCall("{CALL sp_UsuarioSystem_Update(?,?,?,?,?,?,?,?,?,?,?,?)}");
                cs.setInt(1, DniEdit);
                cs.setInt(2, DniActual);
