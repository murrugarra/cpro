 ...
 * @author Usuario
 */
@WebServlet(name = "EditarCatalogoComercial", urlPatterns = {"/EditarCatalogoComercial"})
public class EditarCatalogoComercial extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditarCatalogoComercial</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EditarCatalogoComercial at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        else
        {
            ...
            try {
                ...
                cs = conexion.prepareCall("{CALL sp_CatalogoComercial_Update(?,?,?,?,?,?,?,?,?)}");
                cs.setInt(1, elemento);
                cs.setInt(2, grupo);
                cs.setInt(3, Clase);
                cs.setInt(4, SubClase);
                cs.setString(5, vNombre);
                cs.setInt(6, UMedida);
                cs.setString(7, iPersonaRegistra);
                cs.setString(8, conPrecio);
