 ...
 * @author Usuario
 */
@WebServlet(name = "EditarUnidadMedida", urlPatterns = {"/EditarUnidadMedida"})
public class EditarUnidadMedida extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditarUnidadMedida</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EditarUnidadMedida at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        else
        {
            ...
            try {
                ...
                String Abrev = request.getParameter("Abrev").trim();
                String iPersonaRegistra = sesion.getAttribute("usuario").toString();

                cs = conexion.prepareCall("{CALL sp_UnidadMedida_Update(?,?,?,?,?)}");
                cs.setInt(1, codigo);
                cs.setString(2, vNombre);
                cs.setString(3, Abrev);
